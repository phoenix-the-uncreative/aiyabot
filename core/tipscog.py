...
class TipsView(View):
    ...
    async def button_about(self, button, interaction):
        ...
        embed_about = discord.Embed(title="About me",
                                    description=f"Хули ты пялишь. Думаешь, я спиздил этого бота?\n"
                                                f"[Да.]({url}) Но я сделал для него свою фичу, а значит я почти что "
                                                f"имею право называть себя создателем.")
        embed_about.colour = settings.global_var.embed_color
        embed_about.set_thumbnail(url=url2)
        embed_about.set_footer(text='Этого достаточно.', icon_url=url2)
